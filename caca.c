...
// XXX: https://www.hackerearth.com/practice/data-structures/trees/heapspriority-queues/practice-problems/algorithm/monk-and-champions-league/
#if 1
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
...
static inline void heap_shit_push_up(heap_shit *heap_ctx, natural idx) {
	...
	nodo = heap[idx];
	if (heap_shit_idx_padre(idx)) {
		caca_log_debug("el padre de %u es %s(%u)", idx,
				heap_ctx->elem_a_cad_fn(heap[heap_shit_idx_padre(idx)].valor,CACA_COMUN_BUF_STATICO),
				heap_shit_idx_padre(idx));
	}
	...
	while ((idx_padre = heap_shit_idx_padre(idx))
		...
		caca_log_debug("bajando %s a %u",
				heap_ctx->elem_a_cad_fn(heap[idx_padre].valor,CACA_COMUN_BUF_STATICO),
				idx);
...
typedef struct gpo_fila {
	natural num_libres;
	natural num_filas;
} gpo_fila;

#define MAX_GPOS_FILAS ((natural)1E6)

gpo_fila g[MAX_GPOS_FILAS + 1] = { 0 };
natural usados[MAX_GPOS_FILAS] = { 0 };

int caca_compara_prioridad(void *pa, void *pb) {
	gpo_fila *a = pa;
	gpo_fila *b = pb;
	int r = 0;

	r = a->num_libres - b->num_libres;

	assert_timeout(a == b || r);

	caca_log_debug("comparando %u con %u r %d", a->num_libres, b->num_libres, r);
...
heap_shit_nodo_llave *caca_obten_llave(void *valor,
	...
	llave_res->contenido = &a->num_libres;
	llave_res->contenido_tam = sizeof(a->num_libres);

//	caca_log_debug("regresando llave %u tam %u", *(natural *)llave_res->contenido, llave_res->contenido_tam);

	return llave_res;
}

void *caca_obten_prioridad(void *valor) {
	gpo_fila *a = valor;

	return (void *) (entero_largo_sin_signo) a->num_filas;
}

char *caca_a_cadena(void *valor, char *buffer) {
	gpo_fila *a = valor;
	sprintf(buffer, "libres id:%u,filas:%u", a->num_libres, a->num_filas);
	return buffer;
}
...
void caca_main() {
	...
	natural m = 0;
	heap_shit *h = NULL;
	natural usados_cnt = 0;
	natural r = 0;

	scanf("%u %u\n", &m, &n);

	h = heap_shit_init(falso, caca_compara_prioridad, caca_obten_llave,
			caca_obten_prioridad, caca_a_cadena);
	...
	for (natural i = 0; i < usados_cnt; i++) {
		natural t = usados[i];
		gpo_fila *ga = g + t;

		heap_shit_insertar(h, ga);
	}

	while (n && !heap_shit_vacio(h)) {
		gpo_fila *ga = heap_shit_borra_torpe(h);
		gpo_fila *gs = NULL;
		natural nt = caca_comun_min(ga->num_filas, n);

		caca_log_debug("procesando %u filas con libres %u", ga->num_filas,
				ga->num_libres);

		r += nt * ga->num_libres;

		gs = g + (ga->num_libres - 1);

		gs->num_libres = ga->num_libres - 1;
		gs->num_filas += ga->num_filas;

		if (!heap_shit_contiene_elemento(h, &gs->num_libres,
				sizeof(gs->num_libres))) {
			heap_shit_insertar(h, gs);
		}

		ga->num_libres = (natural) HEAP_SHIT_VALOR_INVALIDO;
		ga->num_filas = (natural) HEAP_SHIT_VALOR_INVALIDO;
		n -= nt;
	}
